...
y_raw <- cbind(1, Z) %*% v
y_intermediate <- matrix(0,nrow=nrow(y_raw),ncol=ncol(y_raw))
y_predicted <- matrix(0,nrow=nrow(y_BinaryTruth),ncol=ncol(y_BinaryTruth))

for (p in 1:nrow(y_raw)){
  for (c in 1:ncol(y_raw)){
    y_predicted[p,c] <- exp(y_raw[p,c])/sum(exp(y_raw[p,]))
  }
}
...
while (1) {
  for (i in sample(N)) {
    ...
    for (c in 1:ncol(y_raw)){
      y_predicted[i,c] <- exp(y_raw[i,c])/sum(exp(y_raw[i,]))
    }
    
    for (k in 1:K){
      v[,k] <- v[,k] + eta * (y_BinaryTruth[i,k] - y_predicted[i,k]) * c(1, Z[i,])
    }
    ...
    for (h in 1:H) {
      ...
      W[,h] <- W[,h] + eta * intermediate * Z[i, h] * (1 - Z[i, h]) * c(1, X[i,])
    }
  }
  ...
  if (abs(objective_values[iteration + 1] - objective_values[iteration]) < epsilon | iteration >= max_iteration) {
    ...
    #one last run for the sample, because I realized that the last round is still important for the classification,
    #and we are using gradient descent after calculating y_predicted
    #long story short : another run with current weights is needed.
    for (i in sample(N)) {
      Z[i,] <- sigmoid(c(1, X[i,]) %*% W)
      y_raw[i,] <- c(1, Z[i,]) %*% v
      for (c in 1:ncol(y_raw)){
