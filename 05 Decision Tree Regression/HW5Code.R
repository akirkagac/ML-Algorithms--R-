...
decisionTree <- function(P){
  ...
  #print RMSE value for given P
  RMSE <- sqrt(sum((y_predicted-y_test)^2)/N_test)
  print(paste0("RMSE is ", RMSE," when P is ", P))
  
  #create output list which we will need later
  treeOutput <- list()
  treeOutput[[1]] <- RMSE
  treeOutput[[2]] <- currentMean
  treeOutput[[3]] <- node_splits
  treeOutput[[4]] <- is_terminal
  
  return (treeOutput)
}


#solving the tree for P=10
outputq1 <- decisionTree(10)
y_mean <- outputq1[[2]]
node_splits <- outputq1[[3]]
is_terminal <- outputq1[[4]]
...
#iterating over different P values to calculate RMSE values
RMSE_value <- c()
for (P in 1:20){
...
plot(1:20,RMSE_value, type="b",ylab = "RMSE", xlab = "P", pch=1,lwd=2)
title("RMSE vs P")
